...
const generalLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, max: 100, standardHeaders: "draft-7",
  message: { status: "error", message: "Too many requests. Please try again later." },
});
const authLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, max: 10, standardHeaders: "draft-7",
  message: { status: "error", message: "Too many login/signup attempts. Please try again after 15 minutes." },
});
const contactLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, max: 5, standardHeaders: "draft-7",
  message: { status: "error", message: "Too many submissions. Please try again later." },
});
const orderLimiter = rateLimit({
  windowMs: 60 * 60 * 1000, max: 10, standardHeaders: "draft-7",
