...
export default function Cart() {
  ...
  const handleDeleteCart = (id) => updateCart(cart.filter(prod => prod._id !== id));

  const handleQuantity = (id, delta) => updateCart(
    cart.map(prod => prod._id === id ? { ...prod, quantity: Math.max(1, (prod.quantity || 1) + delta) } : prod)
  );
  ...
  return (
    <>
      ...
      {!loginDetail ? (
        <div className="cart-empty"><h3>Please log in to view your cart.</h3></div>
      ) : cart.length === 0 ? (
        ...
        <div className="cart-container">
          ...
          <button className="proceed-payment" onClick={() => navigate("/payment")}>Proceed to Payment</button>
        </div>
      )}
    </>
  );
